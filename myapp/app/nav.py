...
ITEMS = [
    ...
    # 준비도 하나에 두 가지 보기가 있다(고객사 하나 깊게 / 전부 넓게).
    # 예전에는 메뉴가 둘이었는데, 점검 항목이 같아서 "어느 쪽을 봐야 하지"
    # 를 매번 묻게 됐다. 같은 자료의 두 보기라면 탭이 맞다.
    {"endpoint": "customer.readiness_page", "label": "준비 상태", "icon": "◎",
     "category": "customer", "match": ("customer.readiness", "customer.settings"),
     "hint": "무엇이 비어 있나 — 한 곳 깊게 / 전부 넓게"},
    {"endpoint": "customer.brief_page", "label": "인수인계", "icon": "⊞",
     "category": "customer", "match": ("customer.brief",),
     "hint": "이 고객사에 대해 아는 것을 한 장으로"},
    {"endpoint": "customer.access_page", "label": "계정 접속", "icon": "⚿",
     "category": "customer", "match": ("customer.access",),
     "hint": "고객사 계정에 들어갈 수 있는가"},
    {"endpoint": "customer.standards_page", "label": "구성 표준", "icon": "⌗",
     "category": "customer", "match": ("customer.standards",),
     "hint": "이 고객사와 합의한 구성인가"},
