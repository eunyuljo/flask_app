...
-- 고객사별 구성 표준
-- ----------------------------------------------------------------------
-- 컴플라이언스(app/compliance.py)는 '누구에게나 통하는 모범사례' 다.
-- 여기는 '이 고객사와 합의한 것' 이다. 필수 태그도, 쓸 수 있는 인스턴스
-- 타입도 고객사마다 다르다. 그걸 코드에 박아 두면(REQUIRED_TAGS 가 실제로
-- 그랬다) 고객사가 둘째부터 맞지 않는다.
--
-- ── 규칙 vs 판정 결과 ──────────────────────────────────────────
-- 여기에는 규칙만 둔다. 판정은 스냅샷을 놓고 매번 다시 계산한다.
-- 결과를 저장하면 리소스를 고친 뒤에도 옛 판정이 남아 진실이 둘이 된다
-- (컴플라이언스·온보딩 준비도와 같은 판단).
--
-- ── 평가할 수 있는 것만 넣는다 ────────────────────────────────
-- 규칙 종류는 app/standards.py 의 RULES 에 고정되어 있다. 수집기가
-- 담지 않는 속성으로 규칙을 만들면 "위반 0건" 이 "지키고 있다" 가 아니라
-- "볼 수가 없다" 라는 뜻이 되는데, 화면에서는 똑같이 0 으로 보인다.
-- ======================================================================

CREATE TABLE IF NOT EXISTS customer_standards (
    id         BIGSERIAL   PRIMARY KEY,

    customer   TEXT        NOT NULL,
    -- app/standards.py 의 RULES 키. 값의 형식도 거기서 정한다.
    rule       TEXT        NOT NULL,
    value      TEXT        NOT NULL DEFAULT '',

    -- 왜 이렇게 합의했는지. 다음 사람이 규칙만 보고 되돌리지 않도록.
    note       TEXT        NOT NULL DEFAULT '',

    active     BOOLEAN     NOT NULL DEFAULT true,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),

    -- 같은 고객사에 같은 규칙이 둘일 이유가 없다.
    UNIQUE (customer, rule)
);
