...
1. [목차](#목차)
2. [무엇을 하는 앱인가](#무엇을-하는-앱인가)
3. [프로젝트 구조](#프로젝트-구조)
4. [설치 및 실행](#설치-및-실행)
5. [화면과 URL](#화면과-url)
6. [이벤트 질의어](#이벤트-질의어)
7. [블루프린트 구조 이해하기](#블루프린트-구조-이해하기)
8. [AI 에이전트](#ai-에이전트)
9. [리소스 변경 추적](#리소스-변경-추적)
10. [운영 리포트](#운영-리포트)
11. [AWS 콘솔 (다중 계정)](#aws-콘솔-다중-계정)
12. [알람 AI 진단](#알람-ai-진단)
13. [작업 기록 (변경 증적)](#작업-기록-변경-증적)
14. [변경 승인과 작업창](#변경-승인과-작업창)
15. [런북](#런북)
16. [당직 인계](#당직-인계)
17. [장애 사후 보고서 (RCA)](#장애-사후-보고서-rca)
18. [알람 노이즈](#알람-노이즈)
19. [고객사 현황](#고객사-현황)
20. [테스트](#테스트)
21. [이벤트의 계정 귀속](#이벤트의-계정-귀속)
22. [운영 상태 (배치와 연동)](#운영-상태-배치와-연동)
23. [로그인과 권한](#로그인과-권한)
24. [감사 로그](#감사-로그)
25. [알람 확인 (ack)](#알람-확인-ack)
26. [최초 대응 시간 (SLA)](#최초-대응-시간-sla)
27. [데이터 보존](#데이터-보존)
28. [Slack 연동](#slack-연동)
29. [에스컬레이션](#에스컬레이션)
30. [지난 장애를 진단 재료로](#지난-장애를-진단-재료로)
31. [리소스 목록](#리소스-목록)
32. [컴플라이언스 점검](#컴플라이언스-점검)
33. [메뉴와 카테고리](#메뉴와-카테고리)
34. [월간 서비스 리뷰 (MSR)](#월간-서비스-리뷰-msr)
35. [온보딩 준비도](#온보딩-준비도)
36. [사후 보고서 AI 초안](#사후-보고서-ai-초안)
37. [Lambda 이벤트 정규화](#lambda-이벤트-정규화)
38. [환경변수 전체 목록](#환경변수-전체-목록)
39. [알려진 한계](#알려진-한계)
...
| `/admin/health` | GET | 관리자 | 배치 실행 상태와 연동 점검 |
| `/admin/health/check/<항목>` | POST | 관리자 | 연동 하나를 실제로 두드려 봄 |
| `/work/<번호>/approve` | POST | **관리자** | 작업 승인 (자기 요청 제외) |
| `/work/<번호>/reject` | POST | **관리자** | 작업 반려 (사유 필수) |
...
| `/alarm/` | GET | 필요 | 이벤트 목록 (`?unacked=1`, `?severity=`) |
| `/alarm/<이벤트>/ack` | POST | 필요 | 알람 확인 |
| `/alarm/<이벤트>/unack` | POST | 필요 | 확인 되돌리기 |
...
| `/incident/<번호>/rca/draft` | POST | 필요 | 원인 분석 AI 초안 (저장 안 함) |
| `/incident/<번호>/rca/discard` | POST | 필요 | 초안 버리기 |
| `/incident/<번호>/jira` | POST | 필요 | 제출된 보고서를 Jira 이슈로 |
...
`work_orders` 는 `open` 에서 시작했습니다. **만들어지자마자 작업해도 되는
상태**입니다. 고객 승인 없이 프로덕션을 건드리는 MSP 는 없는데, 그 앞
단계가 통째로 비어 있었고 그래서 **"승인 없이 실행된 작업" 을 찾아낼 수
없었습니다.**

```
예전:                    open → before_taken → after_taken → closed
지금:  requested → open(승인됨) → before_taken → after_taken → closed
                ↘ rejected
```

`open` 의 뜻은 바꾸지 않았습니다. 이미 쌓인 기록이 전부 `open` 이라,
이름을 바꾸면 **지난 작업이 미승인으로 보입니다.**

### 자기 요청은 자기가 승인할 수 없습니다

이 규칙 하나가 나머지를 지탱합니다. **승인이 형식만 남으면 없는 것과
같습니다.**

```sql
UPDATE work_orders SET status = 'open', approved_by = %s, ...
 WHERE id = %s AND status = 'requested' AND requested_by <> %s
```

판정을 SQL 의 `WHERE` 로 합니다. 두 사람이 거의 동시에 눌러도 한 번만
통과해야 하는데, 파이썬에서 상태를 읽고 판단한 뒤 `UPDATE` 하면 **그 사이가
열려 있습니다.** [작업 증적](#작업-기록-변경-증적)의 상태 전이와 같은 방식입니다.

여기서 [역할](#로그인과-권한)이 처음으로 진짜 의미를 갖습니다 &mdash;
operator 가 요청하고 admin 이 승인합니다. 그전까지 역할은 "관리자 화면을
볼 수 있나" 에만 쓰였습니다.

반려에는 **사유가 필수**입니다. 없으면 요청자가 무엇을 고쳐야 할지 모릅니다.

### 승인 전에는 스냅샷도 못 찍습니다

찍을 수 있으면 승인 단계가 장식이 됩니다. `check_transition()` 이
`requested` 를 거부합니다.

### 작업창은 막지 않습니다

작업창(`window_start` ~ `window_end`)을 벗어나도 **거부하지 않습니다.**

> 막으면 급할 때 이 도구를 통째로 우회하고, 그러면 **증적이 아예 안 남습니다.**

대신 벗어났다는 사실을 `out_of_window` 에 남기고, 목록·상세·증적에
그대로 표시합니다. 되돌리지 않습니다.

### 되돌리는 방법을 먼저 봅니다

승인 화면이 `rollback` 칸을 맨 위에 둡니다. **되돌릴 수 없는 작업은
승인하기 전에 다시 물어야 합니다.**

시각은 전부 UTC 입니다. `<input type="datetime-local">` 은 시간대 없이
`2026-08-22T14:30` 을 보내는데, 어디 시각인지 적혀 있지 않으므로 이 앱의
다른 모든 시각과 같이 UTC 로 읽고 **화면에도 UTC 라고 밝힙니다.**
...
`/admin/health` &mdash; 관리자 화면. **내 도구가 지금 제대로 돌고 있나.**

### 배치가 조용히 죽어도 몰랐습니다

이 앱의 자동화는 전부 CLI 입니다. `compliance-check`, `sla-check`,
`handover`, `collect-resources`, `prune-events`. cron 에 걸어두고 죽으면
아무도 모릅니다.

**무서운 건 실패가 아니라 침묵입니다.** 컴플라이언스 점검이 3주째 안 돌아도
화면은 3주 전 스냅샷 기준 결과를 아무 표시 없이 보여줍니다. 읽는 사람은
그게 오늘 상태라고 믿습니다.
...
@tracked("compliance-check")          # ← 이 한 줄
def compliance_check(...):
```

예외가 나면 `failed` 로 남기고 **그대로 다시 던집니다.** 기록 때문에 실패를
삼키면 cron 이 성공한 줄 압니다.

| 상태 | 뜻 |
|---|---|
| 오래됨 | 돌아야 할 주기를 넘겼습니다 &mdash; cron 이 조용히 죽은 자리 |
| 연속 실패 | 한 번 실패는 흔합니다. **연속은 다른 문제**입니다 |
| 멈춤 | 시작만 하고 안 끝났습니다. 프로세스가 사라졌을 수 있습니다 |
| 기록 없음 | 한 번도 안 돌았습니다 |

기록을 **쌓습니다.** 마지막 것만 덮어쓰면 "어제도 실패했나" 를 알 수 없습니다.

> DB 가 죽어 있는 동안 난 실패는 여기 남지 않습니다 &mdash; 기록할 곳이
> 같이 죽어 있기 때문입니다. 그럴 때는 '오래됨' 으로 나타납니다.

`add-account` 처럼 필요할 때만 부르는 명령에는 붙이지 않았습니다.
**그건 "안 돌았다" 가 문제가 아닙니다.**

### 설정됨은 작동함이 아닙니다

관리자 화면은 Slack/Jira 를 "설정됨 / 미설정" 으로만 보여줬습니다. 설정됐다고
되는 게 아닙니다 &mdash; 토큰이 만료되고, 웹훅이 지워지고, 채널에서 앱이
빠집니다. 그런 일은 대개 **SLA 위반 알림이 안 갈 때** 알게 됩니다.

여섯 가지를 실제로 두드려 봅니다: DB, 이벤트 정규화, AI 경로, AWS CLI,
Slack, Jira.

**화면을 열 때 자동으로 돌리지 않습니다.** Slack 에 메시지를 보내고 Jira 를
두드리는 일이라, 새로고침 한 번에 그게 나가면 안 됩니다.

보내는 것은 최소한으로 합니다.

| 점검 | 하는 일 |
|---|---|
| Slack | **실제로 보냅니다** (웹훅은 조회 API 가 없습니다). 점검이라고 밝히는 문구를 함께 |
| Jira | 이슈를 만들지 않고 `/myself` 로 인증만 확인 &mdash; **점검 때문에 티켓이 쌓이면 아무도 안 누릅니다** |
| AI 경로 | 설정만 확인. 점검 한 번에 토큰 비용이 나가면 안 됩니다 |
| Lambda | `aws` 모드면 호출하지 않습니다. 가짜 이벤트가 적재되면 안 됩니다 |

바깥으로 보낸 점검은 [감사 로그](#감사-로그)에 남습니다 &mdash; Slack 채널에
뜬 메시지를 보고 "이거 누가 보냈어?" 라고 물었을 때 답할 수 있어야 합니다.

### 예외를 감싸는 자리

점검 함수는 `(성공 여부, 설명)` 만 돌려줍니다. 예외 처리와 시간 재기는
`run_check()` 한 곳에서 합니다.

처음에는 각 점검이 스스로 `try` 를 썼는데, **나중에 추가하는 사람이
빠뜨리면 화면 전체가 500** 이 됩니다. 정작 다른 연동이 멀쩡한지도 못 보게
되고요. 테스트가 그걸 봅니다.
...
`/alarm/` 의 **확인** 버튼. SLA 의 "최초 대응 시각" 이 여기서 나옵니다.
...
둘은 다릅니다. 콘솔에 들어가 **다른 일**을 해도 감사 기록이 남으므로,
추론은 실제보다 대응이 빨랐던 것처럼 보이게 만들 수 있습니다. 반대로 이
도구를 거치지 않고 대응하면 영원히 '미대응' 으로 남습니다.

### 그렇다고 추론을 버릴 수는 없습니다

버리면 **지난 기록이 전부 미대응**이 됩니다. 그래서 둘 다 쓰되,
`COALESCE(acknowledged_at, 첫 감사기록)` 으로 확인 기록을 우선하고,
**어느 쪽에서 나온 값인지를 세어서** 화면에 밝힙니다.

> 대응 시각 95건 중 **0건은 확인 버튼**, 95건은 감사 로그에서 추론했습니다.

확인 버튼이 자리를 잡을수록 추론 비중이 줄어드는 것이 보여야 합니다.
추론이 확인보다 많으면 화면이 그 사실을 함께 알립니다 &mdash;
**지표를 그만큼 덜 믿어야 한다는 뜻**이기 때문입니다.

### 먼저 본 사람이 최초 대응자

이미 확인된 알람은 덮어쓰지 않습니다. 나중 사람이 덮어쓰면 최초 대응
시각이 뒤로 밀려 지표가 나빠집니다.
...
되돌리기는 둡니다 &mdash; 잘못 누르는 일은 반드시 생깁니다.

### 미확인이 목록보다 위에 옵니다

목록만 보면 무엇이 아직 아무도 안 본 것인지 알 수 없고, **그게 SLA 가
깨지는 자리**입니다. 심각도별로 나눠서 셉니다 &mdash; info 100건보다
critical 1건이 급합니다.

기본 보기를 '미확인만' 으로 두지는 않았습니다. 목록을 열었는데 비어 있으면
**알람이 안 들어오는 것과 구분이 안 되기** 때문입니다.
...
`/resources/inventory` &mdash; **지금 무엇이 떠 있는가.**

[리소스 변경](#리소스-변경-추적)은 두 시점의 **차이**를 보여줍니다.
그건 "무엇이 바뀌었나" 에는 답하지만 "무엇이 있나" 에는 답하지 않습니다.
인수인계·감사·비용 대화에서 매번 요구되는 건 뒤쪽입니다.

> "이 계정에 인스턴스가 몇 대죠?"
> "Env 태그가 없는 리소스 목록 주세요"

**새로 수집할 것이 없습니다.** `resources` 테이블에 이미 다 있습니다.

- 계정+리전마다 **가장 최근 완료 스냅샷 하나**를 봅니다. 여러 스냅샷을
  합치면 이미 지워진 리소스가 살아 있는 것처럼 보입니다.
- 태그 키는 코드에 적지 않고 **실제로 들어 있는 것**을 뽑아서 보여줍니다.
  고객사마다 태그 규칙이 다르기 때문입니다.
- **"이 태그가 없는 것만"** 필터가 따로 있습니다. 비용 배분과 담당자 확인에서
  가장 자주 나오는 질문이라서요. 태그를 못 다는 리소스는 여기서 뺍니다.
- 엑셀은 **종류마다 시트를 나눕니다.** 한 시트에 다 넣으면 종류마다 의미 있는
  열이 달라 빈 칸이 많아지고, 그러면 엑셀에서 걸러볼 수가 없습니다.
...
| critical | **인스턴스 메타데이터 v1 이 열려 있음** | CIS AWS 5.6 |
...
### "위반 0건" 이 두 가지 뜻을 갖던 문제

이게 이 화면에서 가장 나빴던 부분입니다.

```
RDS 위반 0건  →  (1) RDS 를 봤는데 문제가 없다
              →  (2) RDS 를 아예 수집하지 않았다
```

화면에서는 둘 다 `0` 으로 똑같이 보였습니다. **안전하다고 읽히지만 사실은
안 본 것**입니다. 실제로 점검 항목 세 개(RDS 둘, IAM 하나)가 수집기가
만들지 않는 리소스를 보고 있어서 **영원히 걸리지 않는 상태**였습니다.

지금은 점검 항목마다 필요한 리소스 종류를 적어두고, 스냅샷에는
**무엇을 수집했는지**를 함께 남깁니다.

```python
{"id": "rds-public", "requires": ("rds:instance",), ...}
```
...
| 상황 | 판정 |
|---|---|
| 수집했고 0건 | **통과** — 봤는데 문제가 없다 |
| 수집 안 함 | **점검하지 못함** — 보지 않았다 |

빈 배열은 이 열이 생기기 전에 찍힌 스냅샷입니다. 그때는 **실제로 들어 있는
종류만 수집한 것으로** 봅니다 — 없는 종류를 '깨끗함' 으로 처리하는 것보다
'못 봤음' 으로 두는 편이 안전하기 때문입니다.

화면·엑셀·PowerPoint·CLI 전부 이 구분을 그대로 나릅니다.

### API 를 안 늘리고 얻은 점검

`describe_instances` 응답에 **이미 들어 있는데 안 쓰던 필드**가 있었습니다.

| 필드 | 쓰임 |
|---|---|
| `MetadataOptions.HttpTokens` | IMDSv1 이 열려 있는가 |
| `PublicIpAddress` | 인터넷에서 정말 닿는가 |
| `IamInstanceProfile` | 훔칠 자격증명이 있는가 |

**IMDSv1** 은 단순 GET 으로 임시 자격증명을 돌려줍니다. 애플리케이션에
SSRF 하나만 있어도 인스턴스 역할의 자격증명이 통째로 나갑니다. v2 는
토큰을 먼저 받게 해서 그 경로를 막습니다.

붙은 역할이 있으면 **훔칠 것이 있다는 뜻**이라 무게를 올립니다. 이걸 위해
점검 함수가 `(리소스, 설명, 심각도)` 세 개를 내놓을 수 있게 했습니다.

`PublicIpAddress` 는 기존 교차 점검을 정확하게 만듭니다.

| 상태 | 판정 |
|---|---|
| 관리 포트 열림 + 퍼블릭 IP 있음 | **critical** — 지금 인터넷에서 닿습니다 |
| 관리 포트 열림 + 퍼블릭 IP 없음 | medium — 직접 닿지는 않습니다 |
| 퍼블릭 IP 를 수집하기 전 스냅샷 | critical, 단 확인 못 했다고 밝힘 |
...
`/incident/<번호>` &mdash; **AI 초안 만들기** 버튼.

타임라인은 원래 앱이 모아 줬지만, **원인 분석 본문은 사람이 빈 칸에 손으로
썼습니다.** 이제 그 빈 칸을 채운 초안을 만들 수 있습니다.

### 알람 진단과 무엇이 다른가

| | 알람 AI 진단 | 사후 보고서 초안 |
|---|---|---|
| 대상 | 방금 들어온 알람 | 이미 지난 장애 |
| AWS 조회 | **한다** (`aws_read` 도구) | **안 한다** |
| 근거 | 지금 계정 상태 | DB 에 모인 것 전부 |
| 감사 기록 | 고객사 계정을 건드림 | 우리 DB 로 문서를 만듦 |

장애는 이미 지난 일이고 근거는 전부 DB 에 있습니다. 그래서 **도구를 붙이지
않은 단순 호출**이면 됩니다 &mdash; 계정을 고를 필요도 없습니다.

### 무엇을 근거로 삼는가

```
자동 조립된 타임라인   알람 · 작업 기록 · 리소스 변경을 한 시간축에
같은 지문의 지난 장애   재발인지 아닌지가 가장 크게 갈리는 지점
관련 런북              그때 정해둔 대응 절차
사람이 이미 쓴 칸       이미 파악된 것을 다시 추측하지 말라는 뜻
```

근거가 길면 자르되, **잘랐다고 밝힙니다.** 말없이 자르면 모델이 "이게 전부"
로 읽고 없는 것을 근거로 결론을 냅니다.

### 저장은 사람이 누릅니다

초안은 **DB 에 들어가지 않습니다.** 화면의 빈 칸에 채워서 보여줄 뿐이고,
저장 버튼을 눌러야 남습니다. 모델이 쓴 글이 확인 없이 고객사 보고서로
나가면 안 되기 때문입니다.

- **이미 쓴 칸은 건드리지 않습니다.** 초안 버튼을 잘못 눌렀다고 써둔 문장이
  날아가면 안 됩니다.
- 저장하면 초안은 사라집니다. 남겨두면 저장한 내용 위에 또 초안이 얹힌 것처럼
  보입니다.
- 초안 그대로 두고 나가면 그냥 없어집니다. 저장 안 한 초안이 남아 있을
  이유가 없습니다.

### 모르는 것을 모른다고 말하게 합니다

네 칸 말고 **`uncertain`** 을 하나 더 받습니다.

> "규칙 회수 시각이 기록에 없어 조치 시점을 확정할 수 없습니다."

**무엇을 확신 못 하는지 말하지 않는 AI 초안은 위험합니다.** 읽는 사람이
초안을 결론으로 읽기 때문입니다. 이 목록은 화면에서 빨간 칸으로 따로
보여주고, 감사 기록에도 함께 남깁니다.

시스템 프롬프트에도 같은 것을 적어둡니다 &mdash; 근거에 없는 사실을 지어내지
말 것, **시간 순서가 곧 인과는 아니라는 것**, 근거에 있는 작업자와 티켓
번호만 인용할 것.

### 네 칸을 따로 받습니다

마크다운 한 덩어리로 받아서 제목으로 쪼개면, 모델이 제목을 조금만 다르게
써도 칸이 비어버립니다.
...
스키마의 칸 이름은 `incident.NARRATIVE_FIELDS` 와 같아야 합니다 &mdash;
갈라지면 초안이 저장되지 않습니다. 테스트가 그걸 봅니다.

### Jira 로 넘기기

`/incident/<번호>/jira` &mdash; **제출된 보고서만** 넘깁니다. 초안 상태로
넘기면 나중에 내용이 바뀌는데 Jira 쪽은 그대로 남아 **두 곳의 내용이
갈립니다.**

Markdown 을 그대로 보내면 안 됩니다. Jira Cloud REST v3 의 `description` 은
ADF 이고 `app/jira.py` 의 `_adf()` 는 평문을 문단으로만 감쌉니다 &mdash;
`#` 과 `|` 가 글자 그대로 찍힙니다. 그래서 `rca.to_jira()` 를 따로 뒀습니다.
[당직 인계](#당직-인계)에서 Slack 용 렌더러를 따로 둔 것과 같은 이유입니다.

이슈 키는 `incidents.jira_key` 에 남습니다. **버튼을 두 번 누르는 일은 반드시
생기므로**, 라우트에서 한 번 막고 `set_jira_key()` 의 `WHERE jira_key = ''`
로 한 번 더 막습니다(두 요청이 거의 동시에 오면 라우트에서는 못 막습니다).

> 이슈는 만들어졌는데 키를 기록하지 못하면 화면에 그 사실을 알립니다.
> 안 그러면 다음 사람이 또 만듭니다.
...
- **사후 보고서 AI 초안의 실제 모델 호출도 미검증**입니다. 모델 호출만 가짜로
  바꿔 검증했습니다 &mdash; 근거 조립·JSON 스키마·거절 처리·초안이 저장되지
  않는 것·이미 쓴 칸을 덮지 않는 것·감사 기록은 실제 코드로 검증했습니다.
- **Jira 이슈 생성도 미검증**입니다. 평문 렌더링·중복 방지·제출 전 거부는
  검증했고, 실제 Jira 호출만 가짜로 바꿨습니다.
